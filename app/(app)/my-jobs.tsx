...
export default function ActiveJobsPage() {
  ...
  const loadJobs = useCallback(async (isRefresh = false, status = "Active") => {
    try {
      ...
      <View style={[styles.jobCard, { backgroundColor: theme.card, borderLeftColor: theme.primary }]}>
        <View style={styles.jobHeader}>
          <View style={styles.jobTitleRow}>
            ...
            <View style={[
              ...
              <Text style={[styles.priorityText, { color: statusInfo.color }]}>{item.status}</Text>
            </View>
          </View>
          <Text style={[styles.jobTitle, { color: theme.text }]}>{item.title || item.description || "Job Title"}</Text>
        ...
        <View style={styles.jobDetails}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={[styles.detailText, { color: theme.textSecondary }]}>{getTenantName()}</Text>
            {getTenantPhone() && (
              <TouchableOpacity style={[styles.phoneButton, { backgroundColor: theme.background }]} onPress={handlePhonePress}>
                <MaterialCommunityIcons name="phone" size={16} color={theme.primary} />
              </TouchableOpacity>
            )}
          </View>

          <View style={styles.detailRow}>
            <MaterialCommunityIcons name="account-hard-hat" size={16} color={theme.textSecondary} />
            <Text style={[styles.detailText, { color: theme.textSecondary }]}>Technician: Juhan</Text>
          </View>
        </View>
        ...
        <View style={styles.jobActions}>
          {(item.status === "Scheduled" || item.status === "In Progress") && canCompleteJob() && (
            <TouchableOpacity style={[styles.completeButton, { backgroundColor: theme.primary }]}>
  ...
  const renderEmptyState = useCallback(() => (
    <View style={styles.emptyState}>
      <MaterialCommunityIcons name="clipboard-list" size={64} color={theme.textTertiary} />
      <Text style={[styles.emptyTitle, { color: theme.text }]}>No {selectedStatus} Jobs</Text>
